...
function render_schedule_form(array $defaults = [], array $options = []): void
{
    ...
    if ($showScopedRole) {
        ...
        $faculty = $st->fetchAll();
        if (is_ge_program_scope($programScope) && db_column_exists('faculty', 'is_gened')) {
            $faculty = db()->query(
                "SELECT id, faculty_id, full_name FROM faculty WHERE status='active' AND COALESCE(is_gened,0)=1 ORDER BY full_name"
            )->fetchAll();
        }
        ...
        $courses = $st->fetchAll();
        if (is_ge_program_scope($programScope) && $collegeId && db_column_exists('courses', 'is_gened')) {
            $existingIds = array_flip(array_map(static fn ($c) => (int) $c['id'], $courses));
            $blockCols = $showBlockScope ? ', year_level, section' : '';
            foreach (ge_courses_offered_to_college($collegeId) as $geCourse) {
                $gid = (int) ($geCourse['id'] ?? 0);
                if ($gid < 1 || isset($existingIds[$gid])) {
                    continue;
                }
                $courses[] = [
                    'id' => $gid,
        ...
        $rooms = $st->fetchAll();
        if (is_ge_program_scope($programScope) && db_column_exists('rooms', 'is_gened')) {
            $rooms = db()->query(
                "SELECT id, room_code, room_name FROM rooms WHERE status IN ('available','tba') AND COALESCE(is_gened,0)=1 ORDER BY room_code"
            )->fetchAll();
        }
    } else {
