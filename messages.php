...
/** Faculty also receive dean/admin memos in internal_messages; badge counts those but classroom UI hid them. */
$showFacultyInbox = is_faculty() && messaging_table_exists();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'send' && (!$usesClassroomDiscussions || $showFacultyInbox)) {
...
$allowInternalThreads = !$usesClassroomDiscussions || $showFacultyInbox;
$withId = $allowInternalThreads ? (int) ($_GET['with'] ?? 0) : 0;
$threadUser = $allowInternalThreads && $withId > 0 ? messaging_user_row($withId) : null;
$canOpen = $allowInternalThreads && $withId > 0 && $threadUser && messaging_can_open_thread($userId, $withId);
if ($canOpen) {
...
$conversations = $allowInternalThreads ? messaging_conversation_list($userId) : [];
$inboxRecipients = $showFacultyInbox ? messaging_faculty_inbox_recipients($userId) : [];
...
$showInternalThread = $canOpen && $threadUser;
$canSendTo = $withId > 0 && $threadUser && ($showInternalThread || !$usesClassroomDiscussions)
    && messaging_can_send($userId, $withId);
    ...
    <div class="col-lg-4">
        <div class="card shadow-sm h-100">
            ...
            <div class="list-group list-group-flush" style="max-height: 70vh; overflow-y: auto;">
                <?php if ($showFacultyInbox): ?>
                    <div class="list-group-item list-group-item-secondary py-1 small fw-semibold text-uppercase">Inbox</div>
                    <?php if (!$conversations): ?>
                        <div class="list-group-item text-muted small">No messages from administration yet.</div>
                    <?php endif; ?>
                    <?php foreach ($conversations as $c): ?>
                        <a class="list-group-item list-group-item-action d-flex justify-content-between align-items-start <?= $withId === (int) $c['partner_id'] ? 'active' : '' ?>"
                           href="messages.php?with=<?= (int) $c['partner_id'] ?>"<?= app_tooltip_attr('Opens your inbox thread with this dean, program chair, or sender. Unread items clear after you open it.') ?>>
                            <div class="me-2">
                                <div class="fw-semibold"><?= htmlspecialchars($c['full_name']) ?></div>
                                <div class="small opacity-75 text-truncate" style="max-width: 220px;"><?= htmlspecialchars($c['preview'] !== '' ? $c['preview'] : 'Open conversation') ?></div>
                            </div>
                            <?php if ($c['unread'] > 0): ?>
                    ...
                    <?php endforeach; ?>
                    <div class="list-group-item list-group-item-secondary py-1 small fw-semibold text-uppercase">Subjects</div>
                <?php endif; ?>
            ...
            <div class="card-body border-top bg-body-tertiary">
                <?php if ($showFacultyInbox): ?>
                    <label class="form-label small text-muted mb-1"><?= messaging_is_gened_faculty_user($userId) ? 'Message Dean Francis Alterado or GEN ED' : 'Message dean, program chair, or GEN ED' ?></label>
                    <form method="get" class="d-flex gap-2 flex-wrap mb-3">
                        <select name="with" class="form-select form-select-sm" onchange="if(this.value) window.location='messages.php?with='+this.value">
                            ...
                            <?php foreach ($inboxRecipients as $r): ?>
                    ...
                    <?php if (!$inboxRecipients): ?>
                        <p class="small text-muted mb-0">No dean, program chair, or GEN ED contact is available for your account.</p>
                    <?php endif; ?>
                <?php endif; ?>
                <?php if ($usesClassroomDiscussions): ?>
    ...
    <div class="col-lg-8">
        <div class="card shadow-sm">
            <div class="card-header bg-white d-flex justify-content-between align-items-center flex-wrap gap-2">
                ...
                <?php elseif ($usesClassroomDiscussions && $activeClassroom): ?>
                    <div>
                ...
                <?php else: ?>
                    <span class="text-muted"><?= $showFacultyInbox ? 'Select an inbox message or subject' : ($usesClassroomDiscussions ? 'Select a subject conversation' : 'Select a conversation or recipient') ?></span>
                <?php endif; ?>
            </div>
            <?php if ($showInternalThread && !$canSendTo && $thread): ?>
            ...
            <div class="card-body bg-light" style="min-height: 320px; max-height: 55vh; overflow-y: auto;" id="msgThread">
                <?php if ($showInternalThread): ?>
                        ...
                        <div class="d-flex mb-3 <?= $m['mine'] ? 'justify-content-end' : 'justify-content-start' ?>">
                            <div class="rounded-3 px-3 py-2 shadow-sm <?= $m['mine'] ? 'bg-primary text-white' : 'bg-white border' ?>" style="max-width: 85%;">
                                ...
                                <?php if ($m['attachment_original_name'] !== ''): ?>
                                    <div class="mt-2">
                                        <a class="btn btn-sm <?= $m['mine'] ? 'btn-light' : 'btn-outline-secondary' ?>" href="message_attachment.php?id=<?= (int) $m['id'] ?>"<?= app_tooltip_attr('Downloads or opens the file attached to this message.') ?>>
                    ...
                    <?php endforeach; ?>
                    <?php if ($thread === []): ?>
                        <p class="text-muted text-center py-5 mb-0">No messages in this conversation yet.</p>
                    <?php endif; ?>
                <?php elseif ($usesClassroomDiscussions && $activeClassroom): ?>
                ...
                <?php elseif ($canOpen && $threadUser): ?>
                    ...
                    <?php endforeach; ?>
                <?php else: ?>
                    <p class="text-muted text-center py-5 mb-0"><?= $showFacultyInbox ? 'Choose an inbox sender or a subject from the left.' : ($usesClassroomDiscussions ? 'Choose a subject from the left to open the shared conversation.' : 'Choose someone from the left to read or send messages.') ?></p>
                <?php endif; ?>
            </div>
            <?php if ($showInternalThread && $canSendTo): ?>
                <div class="card-footer bg-white">
                    <form method="post" class="d-flex flex-column gap-2">
                        <input type="hidden" name="action" value="send">
                        <input type="hidden" name="recipient_id" value="<?= $withId ?>">
                        <label class="visually-hidden" for="msgBodyInbox">Reply</label>
                        <textarea name="body" id="msgBodyInbox" class="form-control" rows="3" placeholder="Type your reply to <?= htmlspecialchars($threadUser['full_name']) ?>…" required maxlength="8000"></textarea>
                        <div class="text-end">
                            <button type="submit" class="btn btn-primary"<?= app_tooltip_attr('Sends your message in this inbox thread. Your dean, program chair, or GEN ED coordinator will see it under Messages.') ?>><i class="fa-solid fa-paper-plane me-1"></i>Send</button>
                        </div>
                    </form>
                </div>
            <?php elseif ($usesClassroomDiscussions && $activeClassroom && $hasClassroomDiscussionTable): ?>
                <div class="card-footer bg-white">
                    <form method="post" class="d-flex flex-column gap-2">
...
<?php if ($showInternalThread || ($usesClassroomDiscussions && $activeClassroom) || ($canOpen && $threadUser)): ?>
<script>
