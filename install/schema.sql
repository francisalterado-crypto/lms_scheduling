...
CREATE TABLE IF NOT EXISTS users (
  ...
  role ENUM('super_admin','admin','dean','program_chair','faculty','gened','student') NOT NULL DEFAULT 'faculty',
  assigned_program VARCHAR(120) NOT NULL DEFAULT '',
  admin_log_title VARCHAR(120) NOT NULL DEFAULT '',
  college_id INT NULL,
...
CREATE TABLE IF NOT EXISTS faculty (
  ...
  employment_status ENUM('Permanent','Contract of Service','Temporary') NOT NULL DEFAULT 'Permanent',
...
CREATE TABLE IF NOT EXISTS schedules (
  ...
  makeup_for_schedule_id INT NULL,
  created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
...
CREATE TABLE IF NOT EXISTS admin_activity_logs (
  ...
  actor_full_name VARCHAR(100) NOT NULL DEFAULT '',
  actor_log_title VARCHAR(120) NOT NULL DEFAULT '',
  user_role VARCHAR(32) NOT NULL DEFAULT 'admin',
  action_type VARCHAR(20) NOT NULL,
  target_module VARCHAR(120) NOT NULL,
  target_reference VARCHAR(255) NOT NULL DEFAULT '',
  details_json LONGTEXT NULL,
  created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
  KEY idx_admin_activity_created (created_at),
  KEY idx_admin_activity_user_created (admin_user_id, created_at),
  KEY idx_admin_activity_role_created (user_role, created_at),
  CONSTRAINT fk_aal_admin_user FOREIGN KEY (admin_user_id) REFERENCES users(id) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
CREATE TABLE IF NOT EXISTS schedule_change_requests (
  ...
  request_type ENUM('change','makeup') NOT NULL DEFAULT 'change',
  message TEXT NOT NULL,
  proposed_day_of_week SET('Monday','Tuesday','Wednesday','Thursday','Friday','Saturday','Sunday') NULL,
  proposed_start_time TIME NULL,
  proposed_end_time TIME NULL,
  proposed_room_id INT NULL,
  status ENUM('pending','approved','rejected') NOT NULL DEFAULT 'pending',
...
CREATE TABLE IF NOT EXISTS online_classrooms (
  ...
  syllabus_stored_name VARCHAR(255) NULL,
  syllabus_original_name VARCHAR(255) NULL,
  syllabus_mime VARCHAR(120) NULL,
  banner_stored_name VARCHAR(255) NULL,
  banner_original_name VARCHAR(255) NULL,
  banner_mime VARCHAR(120) NULL,
