...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        if ($action === 'add') {
            ...
            $st = db()->prepare($sql);
            $st->execute($params);
            log_dean_activity('course_delete', 'Deleted course ID #' . (int) $_POST['id']);
...
<div class="app-card">
    ...
    <form method="get" class="row g-3 align-items-end">
        <div class="col-md-5">
            ...
            <input id="courses-q" type="search" name="q" class="form-control" placeholder="Code, name, program, year, section<?= $hasClassroomCode ? ', classroom code' : '' ?>" value="<?= htmlspecialchars($search) ?>" autocomplete="off">
        </div>
        <div class="col-md-3">
    ...
    <table class="table app-table mb-0">
        ...
        <?php foreach ($list as $r): ?>
            <tr>
                ...
                <td class="app-table-actions text-nowrap">
                    <a class="btn btn-sm btn-outline-primary" href="courses.php?edit=<?= (int) $r['id'] ?>" title="Edit course" aria-label="Edit course"<?= app_tooltip_attr('Opens this course for editing. Use this to fix codes, units, program, or classroom metadata.') ?>><i class="fa-solid fa-pen" aria-hidden="true"></i></a>
                    <form method="post" class="d-inline ms-1" onsubmit="return confirm('Delete this course?');">
                        <input type="hidden" name="action" value="delete">
                        <input type="hidden" name="id" value="<?= (int) $r['id'] ?>">
                        <button type="submit" class="btn btn-sm btn-outline-danger" title="Delete course" aria-label="Delete course"<?= app_tooltip_attr('Permanently removes this course after confirmation. Use this only if no schedules should reference it anymore.') ?>><i class="fa-solid fa-trash" aria-hidden="true"></i></button>
                    </form>
                </td>
            </tr>
        <?php endforeach; ?>
