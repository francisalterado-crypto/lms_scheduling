...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        if ($action === 'add') {
            ...
            $fid = (int) $_POST['id'];
            $stBf = db()->prepare(
                'SELECT f.*, u.username, u.is_active AS user_is_active FROM faculty f INNER JOIN users u ON u.id = f.user_id WHERE f.id=? AND COALESCE(f.is_gened,0)=1'
            );
            $stBf->execute([$fid]);
            $beforeFac = $stBf->fetch(PDO::FETCH_ASSOC);
            $chk = db()->prepare('SELECT user_id FROM faculty WHERE id=? AND COALESCE(is_gened,0)=1');
            $chk->execute([$fid]);
            $userId = (int) ($chk->fetchColumn() ?: 0);
            if ($userId < 1) {
                throw new RuntimeException('GE faculty record not found.');
            }
            ...
            if ($hasEmploymentStatusColumn) {
                db()->prepare(
                    'UPDATE faculty SET faculty_id=?, full_name=?, email=?, max_hours_per_day=?, status=?, employment_status=? WHERE id=? AND COALESCE(is_gened,0)=1'
                )->execute([
                    $newCode,
                    trim((string) ($_POST['full_name'] ?? '')),
                    trim((string) ($_POST['email'] ?? '')),
                ...
                db()->prepare(
                    'UPDATE faculty SET faculty_id=?, full_name=?, email=?, max_hours_per_day=?, status=? WHERE id=? AND COALESCE(is_gened,0)=1'
                )->execute([
                    $newCode,
                    trim((string) ($_POST['full_name'] ?? '')),
                    trim((string) ($_POST['email'] ?? '')),
            ...
            log_user_activity('edit', 'GE Faculty', 'GE Faculty #' . $fid, $beforeFac ? (array) $beforeFac : null, $afterFac ? (array) $afterFac : null);
            $_SESSION['flash'] = 'GE faculty updated.';
        } elseif ($action === 'delete' && isset($_POST['id'])) {
            $fid = (int) $_POST['id'];
            $stDelSnap = db()->prepare(
                'SELECT f.*, u.username FROM faculty f INNER JOIN users u ON u.id = f.user_id WHERE f.id=? AND COALESCE(f.is_gened,0)=1'
            );
            $stDelSnap->execute([$fid]);
            $beforeDelFac = $stDelSnap->fetch(PDO::FETCH_ASSOC);
            $q = db()->prepare('SELECT user_id FROM faculty WHERE id=? AND COALESCE(is_gened,0)=1');
            $q->execute([$fid]);
            $userId = (int) ($q->fetchColumn() ?: 0);
            if ($userId < 1) {
                throw new RuntimeException('GE faculty record not found.');
            }
            db()->prepare('DELETE FROM faculty WHERE id=? AND COALESCE(is_gened,0)=1')->execute([$fid]);
            db()->prepare('DELETE FROM users WHERE id=? AND role="faculty"')->execute([$userId]);
            log_user_activity('delete', 'GE Faculty', 'GE Faculty #' . $fid, $beforeDelFac ? (array) $beforeDelFac : null, null);
            $_SESSION['flash'] = 'GE faculty removed.';
        }
    } catch (Throwable $e) {
        if (db()->inTransaction()) {
            db()->rollBack();
    ...
    header('Location: gened_faculty.php');
    exit;
}
...
if (isset($_GET['edit'])) {
    $st = db()->prepare(
        'SELECT f.*, u.username FROM faculty f LEFT JOIN users u ON u.id=f.user_id WHERE f.id=? AND COALESCE(f.is_gened,0)=1'
    );
...
$st = db()->query(
    'SELECT f.*, u.username, u.is_active
     FROM faculty f
     LEFT JOIN users u ON u.id = f.user_id
     WHERE COALESCE(f.is_gened,0)=1
...
<div class="faculty-dashboard-container">
    <div class="faculty-university-header">
        <div class="faculty-title-section">
            <h1><i class="fa-solid fa-chalkboard-user" style="color:#2c7da0; margin-right:8px;"></i>GE Faculty Management</h1>
    ...
    <div class="faculty-stats-row">
        <div class="faculty-stat-card">
            ...
            <div class="faculty-stat-info"><h3><?= $totalFacultyCount ?></h3><span>Total GE Faculty</span></div>
        </div>
        <div class="faculty-stat-card">
    ...
    <div class="faculty-layout">
        <div class="faculty-card">
            <div class="faculty-card-header">
                <h2><i class="fa-solid fa-user-plus"></i> <?= $editRow ? 'Edit GE faculty' : 'Add GE faculty' ?></h2>
                <a href="gened_courses.php" class="faculty-manage-programs-link"><i class="fa-solid fa-book me-1"></i>GE courses</a>
            </div>
            <form method="post" class="faculty-form-grid">
                ...
                <div class="faculty-input-group">
                    <label>Program</label>
                    <input type="text" value="General Education" readonly>
                </div>
                ...
                <div style="display:flex; justify-content:flex-end; gap: 10px; grid-column: 1 / -1;">
                    <?php if ($editRow): ?><a href="gened_faculty.php" class="btn btn-outline-secondary faculty-cancel-link">Cancel</a><?php endif; ?>
                    <button type="submit" class="faculty-save-btn"<?= app_tooltip_attr($editRow ? 'Saves changes to this GE faculty record. Use this after updating name, ID, or status.' : 'Adds the faculty member to the General Education roster. Use this before assigning GE courses or schedules.') ?>><i class="fa-solid <?= $editRow ? 'fa-pen' : 'fa-save' ?>"></i> <?= $editRow ? 'Update GE Faculty' : 'Save GE Faculty' ?></button>
                </div>
            </form>
        </div>
        ...
        <div class="faculty-card">
            <div class="faculty-card-header">
                <h2><i class="fa-solid fa-list-ul"></i> GE faculty roster</h2>
                <span class="faculty-badge-manage"><i class="fa-solid fa-pen me-1"></i>Manage GE faculty records</span>
            </div>
                ...
                <table class="faculty-table">
                    ...
                    <?php foreach ($list as $r): ?>
                        <tr>
                            ...
                            <td><?= htmlspecialchars((string) ($r['department'] ?? 'General Education')) ?></td>
                            ...
                            <td class="faculty-action-buttons">
                                <a href="gened_faculty.php?edit=<?= (int) $r['id'] ?>" class="faculty-icon-btn" title="Edit GE faculty"><i class="fa-solid fa-pen"></i></a>
                                <form method="post" class="d-inline" onsubmit="return confirm('Delete this GE faculty member? Their GE schedules may be affected.');">
                                    <input type="hidden" name="action" value="delete">
                                    <input type="hidden" name="id" value="<?= (int) $r['id'] ?>">
                                    <button type="submit" class="faculty-icon-btn delete" title="Delete GE faculty"><i class="fa-solid fa-trash-can"></i></button>
                                </form>
                            </td>
                        </tr>
                    <?php endforeach; ?>
                    <?php if ($list === []): ?>
                        <tr><td colspan="<?= $hasEmploymentStatusColumn ? '9' : '8' ?>" class="text-center text-muted py-4">No GE faculty yet. Add one above.</td></tr>
                    <?php endif; ?>
                    </tbody>
                </table>
            </div>
    ...
    <div class="faculty-footer-note">
        <i class="fa-solid fa-database"></i> General Education faculty directory · Real-time management | Western Philippines University
    </div>
</div>
