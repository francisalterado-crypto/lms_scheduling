...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $redirectUrl = 'program_chair_student_registrations.php';
    try {
        if ($action === 'approve_all') {
            ...
            if (($bulk['approved'] ?? 0) > 0) {
                log_dean_activity(
                    ...
                    'Bulk approved ' . (int) $bulk['approved'] . ' student registration(s) for ' . $programScope
                );
            }
            ...
            if (!empty($approval['already_approved'])) {
                ...
                log_dean_activity(
                    ...
                    'Approved student registration for ' . $programScope . ' (request #' . $requestId . ')'
                );
            ...
            log_dean_activity(
                ...
                'Rejected student registration for ' . $programScope . ' (request #' . $requestId . ')'
            );
    ...
    <?php if ($pending === []): ?>
        <div class="card border-0 shadow-sm rounded-4">
            <div class="card-body text-center py-5 text-muted">
                ...
                No pending student registrations for <?= htmlspecialchars($programScope) ?>.
            </div>
        </div>
    <?php else: ?>
        <div class="card border-0 shadow-sm rounded-4">
