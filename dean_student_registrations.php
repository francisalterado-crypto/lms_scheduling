...
$programScope = $programFilter !== '' ? $programFilter : null;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $redirectProgram = trim((string) ($_POST['program_filter'] ?? ''));
    $redirectUrl = 'dean_student_registrations.php';
    if ($redirectProgram !== '' && in_array($redirectProgram, $programs, true)) {
        ...
        if ($action === 'approve' && $requestId > 0) {
            ...
            if (!empty($approval['already_approved'])) {
                ...
                log_dean_activity(
                    ...
                    'Approved student registration (request #' . $requestId . ')'
                );
            ...
            log_dean_activity(
                ...
                'Rejected student registration (request #' . $requestId . ')'
            );
...
$pending = pending_registrations_for_college($collegeId, $programScope);
        ...
        <div>
            ...
            <p class="text-muted mb-0">
                ...
                <?php if ($programFilter !== ''): ?>
                    — <?= htmlspecialchars($programFilter) ?>
                <?php else: ?>
                    — all programs
                <?php endif; ?>
            </p>
            ...
            <div class="col-sm-8 col-md-5">
                ...
                <select class="form-select" id="program" name="program" onchange="this.form.submit()">
                    ...
                    <?php foreach ($programs as $programName): ?>
                        <option value="<?= htmlspecialchars((string) $programName) ?>"
                            <?= $programFilter === (string) $programName ? 'selected' : '' ?>>
                            <?= htmlspecialchars((string) $programName) ?>
                        </option>
                    <?php endforeach; ?>
                </select>
            </div>
    ...
    <?php if ($pending === []): ?>
        <div class="card border-0 shadow-sm rounded-4">
            <div class="card-body text-center py-5 text-muted">
                ...
                No pending student registrations<?= $programFilter !== '' ? ' for this program' : ' in your college' ?>.
            </div>
        </div>
    <?php else: ?>
        <div class="card border-0 shadow-sm rounded-4">
