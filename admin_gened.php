...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        $stU = db()->prepare(
            'SELECT id, username, full_name, email, is_active FROM users WHERE id=? AND role="gened"'
        );
        $stU->execute([$id]);
        $beforeFull = $stU->fetch(PDO::FETCH_ASSOC);
        if (!$beforeFull) {
            throw new RuntimeException('GEN ED account not found.');
        }
        $uname = (string) $beforeFull['username'];
        $displayName = $fullName !== '' ? $fullName : (string) $beforeFull['full_name'];
        ...
        $stAfter = db()->prepare(
            'SELECT id, username, full_name, email, is_active FROM users WHERE id=? AND role="gened" LIMIT 1'
        );
        $stAfter->execute([$id]);
        $afterFull = $stAfter->fetch(PDO::FETCH_ASSOC) ?: [];
        if ($afterFull !== [] && ($generateAndEmail || $resetPassword !== '')) {
            $afterFull['password'] = '[changed]';
        }
        log_admin_activity(
            'edit',
            'GEN ED account',
